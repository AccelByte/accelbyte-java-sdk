...
test_cli:
	...
	bash -c 'sed -i "s/\r//" "$(SDK_MOCK_SERVER_PATH)/mock-server.sh" && \
			...
			sed -i "s/\r//" tests/sh/* && \
			(for FILE in $$(ls tests/sh/*.sh | grep -v run-java-cli-all-unit-test.sh); do docker run -it --rm -u $$(id -u):$$(id -g) -v $$(pwd):/data -w /data --network host -e MODULE_PATH="samples/cli" gradle:jdk17 bash $$FILE || touch test.err; done)'
	[ ! -f test.err ]
