 ...
 * secret. For publisher-game namespace schema : Specify only either platform_token or device_id.
 * Device token grant should be requested along with device_id parameter against game namespace.
 * Another 3rd party platform token grant should be requested along with platform_token parameter
 * against publisher namespace.
 *
 * <p>Supported platforms: - **steamopenid**: Steam's user authentication method using OpenID 2.0.
 * The expected value of the platform token is the URL generated by Steam on web authentication The
 * JWT contains user's active bans with its expiry date. List of ban types can be obtained from
 * /iam/bans..
 *
 * <p>### Endpoint migration guide - **Substitute endpoint:
 * _/iam/v3/oauth/platforms/{platformId}/token [POST]_**
 *
 * @deprecated
 */
@Deprecated
