 ...
 * ## Supported platforms: - **steam**: The ticketâs value is the authentication code returned by
 * Steam. - **steamopenid**: Steam's user authentication method using OpenID 2.0. The ticket's value
 * is URL generated by Steam on web authentication - **facebook**: The ticketâs value is the
 ...
 * Every device that doesânt run Android and iOS is categorized as a device platform. The ticket's
 * value is the deviceâs ID. - **discord**: The ticketâs value is the authorization code
 * returned by Discord OAuth. - **awscognito**: The ticketâs value is the aws cognito access token
 * (JWT). - **epicgames**: The ticketâs value is an access-token obtained from Epicgames EOS
 * Account Service. - **nintendo**: The ticketâs value is the authorization code(id_token)
 * returned by Nintendo OAuth.
 */
@Getter
@Setter
