 ...
 * platform_tokenâs value is the authorization code returned by Google OAuth -
 * **googleplaygames**: The platform_tokenâs value is the authorization code returned by Google
 * play games OAuth - **oculus**: The platform_tokenâs value is a string composed of Oculus's user
 * ID and the nonce separated by a colon (:). - **twitch**: The platform_tokenâs value is the
 * authorization code returned by Twitch OAuth. - **discord**: The platform_tokenâs value is the
 * authorization code returned by Discord OAuth - **android**: The device_id is the Androidâs
 * device ID - **ios**: The device_id is the iOSâs device ID. - **apple**: The platform_tokenâs
 * value is the authorization code returned by Apple OAuth.(We will use this code to generate APP
 * token) - **device**: Every device that doesânt run Android and iOS is categorized as a device.
 * The device_id is the deviceâs ID. - **justice**: The platform_tokenâs value is the designated
 * userâs access token. - **epicgames**: The platform_tokenâs value is an access-token obtained
 * from Epicgames EOS Account Service. - **ps4**: The platform_tokenâs value is the authorization
 * code returned by Sony OAuth. - **ps5**: The platform_tokenâs value is the authorization code
 * returned by Sony OAuth. - **nintendo**: The platform_tokenâs value is the authorization
 * code(id_token) returned by Nintendo OAuth. - **awscognito**: The platform_tokenâs value is the
