 ...
 * Persistent Flag only can use with type DS (example value true or false) - If Persistent True the
 * session always active even DS removing or terminate and Session will be request DS again until DS
 * Ready or Busy. - To Stop Session Not request again to DS or want Delete Session can Delete
 * Session using endpoint DELETE /session/v1/public/namespaces/{namespace}/gamesessions/{sessionId}
 * - If Persistent False the session will be inactive if all member left and DS terminate or
 * removing - nativeSessionSetting: - XboxSessionTemplateName: the XBox session template name that
 ...
 * PSNBaseUrl this is for base URL PSN if not set will be default value
 * https://s2s.sp-int.playstation.net. In a single namespace only 1 PSN Env that can be used.
 * Multiple session template should refers to the same PSN Env as we have in IAM Service. -
 * https://s2s.sp-int.playstation.net (DEV, need IP Whitelist) - https://s2s.prod-qa.playstation.net
 * (QA Environment/PSN Certification) - https://s2s.np.playstation.net (Production) -
 * localizedSessionName : for localized name and default language example payload :
 * "localizedSessionName":{ "defaultLanguage" : "en-US" "localizedText" :{ "en-US" : "title" } }
 */
@Getter
@Setter
