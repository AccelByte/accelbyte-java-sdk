 ...
 *
 * <p>Modify template configuration Session configuration mandatory : - name - joinability.
 * supported values: - OPEN: user can join or rejoin the session without an invite - CLOSED: once a
 * CLOSED session is created or a session joinability is updated to CLOSED, any change to session
 * members is not allowed and remaining invites will all be canceled. - INVITE_ONLY: to join the
 * session, user whether needs to be invited, or have the code (for join by code). when a user is
 * KICKED/LEFT/REJECTED from this session, they will need to be re-invited to join. -
 * FRIENDS_OF_MEMBERS: only if a user is friend with at least one of the session members, they can
 * join via code. - FRIENDS_OF_LEADER: only if a user is friend with the session leader, they can
 * join via code. - FRIENDS_OF_FRIENDS: only friends of the leader OR friends of friends of the
 * leader can join via code. - autoJoin: when enabled, players will automatically join the initial
 * game session creation. Game session will not send any invite and players dont need to act upon
 * it. default: false (disabled) - Type (example value : P2P, DS, NONE) if type empty, type will be
 * assign to NONE - MinPlayers (must greater or equal 0) - MaxPlayers (must greater than 0) -
 ...
 * PSNSupportedPlatforms is empty, then PS5 will be set as default value. - PSNDisableSystemUIMenu:
 * items whose operation from the PSN system software UI is to be disabled. - PSNBaseUrl this is for
 ...
 * reconnection when enabled. - DisableResendInvite (optional, default: false): by default, the
 * service will be sending invites until the user reacts to it. enabling this flag will prohibit the
 * service to send reminder invites'. - appName (optional, default: empty string). Extend
 * application name that uploaded to AccelByte extends deployment. The application is for
 * custom/external DS management. - customURLGRPC (optional: default: empty string). Url for grpc
 * server that server custom/external DS management. This value will be ignore when appName is
 * filled. Value formatted as host:port. Example: yourdomain.com:9989 - grpcSessionConfig (optional,
 * default: empty object). Setting for grpc session extends. Session service will calls the grpc
 * when any activity for the game sessions. - appName (required when customURL is empty) The extends
 * app name that created and uploaded to AccelByte extends deployment. - customURL (required when
 * appName is empty) The url for grpc server. Format is host:port. Example: mycustomdomain.com:9889.
 * This will be ignore when appName is filled. - functionFlag (optional, default: -1) Flag to enable
 * which function to calls when any event on the game sessions. The value is bitwise integer. - 0x1
 * is for created event. Event is sync, anything return from create event will be overide the final
 * game session. - 0x2 is for updated event. Event is async. Called when any update on the game
 * session. - 0x4 is for deleted event. Event is async. Callen when game session got deleted.
 * Example: - value 3: enable for created and updated - value 7: enable for created, updated, and
 * deleted, etc - partyCodeGeneratorString (optional, default: 0123456789ABCDEFGHJKMNPQRSTVWXYZ) The
 * code is generated by randomly selecting characters from the allowed set, with at least 6
 * characters long. Allowed characters: 0123456789ABCDEFGHJKMNPQRSTVWXYZ (All digits 0â9 and
 * uppercase letters AâZ, excluding I, L, O, and U).
 */
@Getter
@Setter
