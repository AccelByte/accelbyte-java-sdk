 ...
 *
 * <p>`resourceStatus` field - indicates the NoSQL cluster status: - `deleting` : The cluster is in
 * the process of being deleted and is not accessible. - `stopped` : The cluster is stopped and not
 * accessible. - `updating` : The cluster is being modified and is not yet accessible (e.g.,
 * updating min/max DCU). - `failed` : The cluster failed to provision or is in an error state and
 ...
 * rebooting. - `maintenance` : The cluster is undergoing maintenance operations and is not
 * accessible. - `unknown` : The cluster status is not recognized - `available` : The cluster is
