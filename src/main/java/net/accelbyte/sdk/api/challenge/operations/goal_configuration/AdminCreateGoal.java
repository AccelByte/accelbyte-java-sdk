 ...
 * describing the goal (optional). * schedule: a time range that indicated the availability of a
 * goal within a timeframe. Used in FIXED assignment rule, this will be required in that case. *
 ...
 * thirdPartyAccountCreated, and userLoggedIn. * For the userAccountLinked event, parameterName will
 * utilize platformId rather than the event name. This will be validated against the list of active
 * third-party accounts configuration within the namespace, such as psn , steam , or device. *
 ...
 * to progress or not. Goal describe set of requirements that need to be fulfilled by players in
 * order to complete it and describe what is the rewards given to player when they complete the
 * goal. The requirement will have target value and a operator that will evaluate that against an
 * observable playerâs attribute (e.g. statistic, entitlement). Goal belongs to a challenge.
 * Supported item type for ENTITLEMENT reward type: APP, BUNDLE, CODE, COINS, EXTENSION, INGAMEITEM,
 * LOOTBOX, MEDIA, OPTIONBOX. Number of goals per challenge is limited to 100 goals.
 */
@Getter
@Setter
