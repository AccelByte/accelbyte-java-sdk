 ...
 * requirementGroups: list of conditions that conform with the goal progressions. * operator:
 * logical operator used to validate the completion of a goal. a goal is considered complete once
 * complete predicates operated with operator result in true. * predicates: list of progression
 * parameters to be tracked. * parameterType: the type of parameter for challenge to be progressed
 * with. the available options are: * STATISTIC: progress by user statistic item value. *
 * STATISTIC_CYCLE: progress user statistic cycle item value. statCycleId must be included. *
 * ACHIEVEMENT: progress by user achievement. * USERACCOUNT: progress by user account event. *
 * ENTITLEMENT: progress by user's item entitlement ownership. Supported item types are APP, CODE,
 * INGAMEITEM, LOOTBOX, MEDIA, and OPTIONBOX. * parameterName: the name of the parameter for
 * challenge to be progressed with. * STATISTIC: refers to stat code. * STATISTIC_CYCLE: refers to
 * stat code with statCycleId must be included. * ACHIEVEMENT: refers to achievement code. *
 * USERACCOUNT: one of the user account event. current possible values are (userAccountCreated,
 * gameUserAccountCreated, userAccountVerified, userAccountLinked,
