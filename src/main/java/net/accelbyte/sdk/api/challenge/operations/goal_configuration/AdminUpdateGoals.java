 ...
 * FIXED assignment rule, this will be required in that case. * requirementGroups: contains array of
 * objects consisting of operator and predicates attribute. When requirements evaluation happened,
 * each item's predicates will be evaluated first, by operating the predicates with given operator.
 * After all items evaluated, the results of the evaluation on each item then will be logically
 * evaluated using OR operator. * operator: logical operator used to validate the completion of a
 * goal. a goal is considered complete once complete predicates operated with operator result in
 * true. * predicates: list of progression parameters to be tracked. * parameterType: the type of
 ...
 * statCycleId must be included. * ACHIEVEMENT: progress by user achievement. * USERACCOUNT:
 * progress by user account event. * ENTITLEMENT: progress by user's item entitlement ownership.
 * Supported item types are APP, CODE, INGAMEITEM, LOOTBOX, MEDIA, and OPTIONBOX. * parameterName:
 * the name of the parameter for challenge to be progressed with. * STATISTIC: refers to stat code.
 * * STATISTIC_CYCLE: refers to stat code with statCycleId must be included. * ACHIEVEMENT: refers
 * to achievement code. * USERACCOUNT: one of the user account event. Current possible values are
 ...
 * given to player when they complete the goal.The requirement will have target value and a operator
 * that will evaluate that against an observable playerâs attribute (e.g. statistic, entitlement).
 * Goal belongs to a challenge.Supported item type for ENTITLEMENT reward type: APP, BUNDLE, CODE,
 * COINS, EXTENSION, INGAMEITEM, LOOTBOX, MEDIA, OPTIONBOX
 */
@Getter
@Setter
