...
public class TestFlightId {
    ...
    public static void setup() throws NoSuchFieldException, IllegalAccessException {
        ...
        wireMockServer.stubFor(any(urlMatching(".*")) // Matches any URL
    ...
    public void testMultipleSdkInstanceHaveSameFlightId() throws Exception {
        ...
        AccelByteSDK sdk1 = new AccelByteSDK(httpClient, new DefaultTokenRepository(), new DefaultConfigRepository());
        AccelByteSDK sdk2 = new AccelByteSDK(httpClient, new DefaultTokenRepository(), new DefaultConfigRepository());
    ...
    public void testHttpHeaderContainsFlightId() throws Exception {
        // given
        AccelByteSDK sdk = new AccelByteSDK(httpClient, new DefaultTokenRepository(), new DefaultConfigRepository());

        // when
        sdk.loginUser("testUser", "testPassword"); // this call will fail, but it's fine since we only need to capture the request header

        // then
        List<LoggedRequest> requests = wireMockServer.findAll(anyRequestedFor(WireMock.urlMatching(".*")));
        boolean allRequestsHaveHeader = requests.stream()
                .allMatch(request -> request.getHeader(HttpHeaders.X_FLIGHT_ID) != null);
    ...
    public void testHttpHeaderForOperationWithCustomFlightId() throws Exception {
        ...
        boolean allRequestsHaveHeader = requests.stream()
                .allMatch(request -> request.getHeader(HttpHeaders.X_FLIGHT_ID) != null);
        boolean isRequestHaveCustomFlightId = requests.stream()
